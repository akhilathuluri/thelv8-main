...
export const paymentService = {
    ...
    async handlePaymentCaptured(payment: any): Promise<void> {
        try {
            ...
            const { error } = await supabaseAdmin
                .from('orders')
                // @ts-expect-error - Database types don't recognize all update fields
                .update({
                    payment_status: 'paid',
    ...
    async findOrderByRazorpayOrderId(razorpayOrderId: string): Promise<any | null> {
        try {
            ...
            const { data: logData, error: logError } = await supabaseAdmin
                ...
                // @ts-expect-error - payment_logs table not in generated types until migration is run
                .single();
            ...
            // @ts-expect-error - logData type not available until migration is run
            if (logData?.order_id) {
                const { data: order, error: orderError } = await supabaseAdmin
                    .from('orders')
                    .select('*')
                    // @ts-expect-error - logData.order_id type not available
    ...
    async getPaymentLogs(orderId: string): Promise<PaymentLog[]> {
        try {
            ...
            if (error) {
                console.error('[PaymentService] Error fetching payment logs:', error);
                return [];
            }
